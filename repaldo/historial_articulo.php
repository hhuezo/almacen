<?php 
	...
	$rs = mysql_query("select YEAR(NOW()) as axo");
	$rowAxo = mysql_fetch_array($rs);	
	mysql_close();
 ?>
...
function consultar(){
	//alert('consultar()');
	buscarAceptar('ajax/historial_articulo_aceptar.php?modoop=1&id_articulo='+kardex.id_articulo.value,'targetDiv');
}	
</script>	
   ...
   <tr>
     <td colspan="2"><center>
       <input type="button" name="btnAceptar" value="Aceptar"   size="10" onClick="
			modoop=1;
				sendQueryToAdd('ajax/historial_articulo_aceptar.php?modoop=1&id_articulo='+kardex.id_articulo.value+
