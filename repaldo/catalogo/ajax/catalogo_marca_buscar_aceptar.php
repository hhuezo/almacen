...
if (isset($_GET["txt_nombre"])) {
  $sql = "SELECT id_marca,nombre_marca FROM marca where nombre_marca like '%".$_GET['txt_nombre']."%'";

$objCon->RetornarRS($result, $sql);
  if ($objCon->ExisteRegistro($sql)){?>
<table border='0' align='center' >
<tr class="row1" bgcolor=#e5eecc><td><b>MARCA</b></td></tr>  
  <?php
	$cont=1;
	  while($rs = $result->fetch_array())
		{?>
		<tr class="<?php echo (($cont % 2) == 0) ? 'row1' : 'row2';?>">
		<td><a href="#" onClick="javascript:marcaSelect('<?php echo $rs[0];?>','<?php echo $rs[1];?>');"><?php echo $rs[1]; ?>
		</a></td>
		</tr>
		<?php
		$cont++;
