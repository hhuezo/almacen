...
if (isset($_GET["txt_nombre"])) {
  $sql = "SELECT id_um,nom_med FROM uni_med where nom_med like '%".$_GET['txt_nombre']."%'";

$objCon->RetornarRS($result, $sql);
  if ($objCon->ExisteRegistro($sql)){?>
<table border='0' align='center' >
<tr class="row1" bgcolor=#e5eecc><td><b>Unidad de medida</b></td></tr>  
  <?php
	$cont=1;
	  while($rs = $result->fetch_array())
		{?>
		<tr class="<?php echo (($cont % 2) == 0) ? 'row1' : 'row2';?>">
		<td><a href="#" onClick="javascript:unidadSelect('<?php echo $rs[0];?>','<?php echo $rs[1];?>');"><?php echo $rs[1]; ?>
		</a></td>
		</tr>
		<?php
		$cont++;
