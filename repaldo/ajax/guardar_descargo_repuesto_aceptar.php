...
if($_GET['txt_numeroSolicitud'] == ''){$txt_numeroSolicitud=0;}
else{$txt_numeroSolicitud=$_GET['txt_numeroSolicitud'];}


//atributos para base de datos de taller
$nom_art=$_GET['txt_articulo'];
$Oid=$_GET['Oid'];
$fecha_sql=$_GET["fecha"];
  	  ...
  	  while($rs = $result->fetch_array())
		{
			...
			if ($cantidad > $c_cantidad){
					...
					//conexion sql para sistema de taller
						...
						$qlInsertSolicitud = "insert into CompraRepuesto(Oid,CompraRepuestos,TipoDocumentos,fecha,NumeroOrdenFactura,NumeroDescargo,Cantidad,UnidadMedida,Descripcion,PrecioUnitario)
						values (NEWID(),'$Oid',1,'$fecha_sql','$c_orden_compra','$descargo',$c_cantidad,'$nombre_medida','$nom_art','$c_precio')";
			...
			else
			{
					...
					//conexion sql para sistema de taller
							...
							//echo 'conexion existe';
							$qlInsertSolicitud = "insert into CompraRepuesto(Oid,CompraRepuestos,TipoDocumentos,fecha,NumeroOrdenFactura,NumeroDescargo,Cantidad,UnidadMedida,Descripcion,PrecioUnitario)
							values (NEWID(),'$Oid',1,'$fecha_sql','$c_orden_compra','$descargo',$cantidad,'$nombre_medida','$nom_art','$c_precio')";
							//echo $qlInsertSolicitud;
							$resulInsertSolicitud = odbc_exec( $conn, $qlInsertSolicitud );
							$sqlUpdate="update SolicitudReparacion set EstadoSolicitud=8 where oid = '$Oid'";
							$resultsqlUpdate = odbc_exec( $conn, $sqlUpdate);
