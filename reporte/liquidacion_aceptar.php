...
<html>
    ...
    <body>
        ...
        if ($Agrupacion == 0) {
            ...
            $ArrayAgrupacion[1][1] = $Agrupacion;
            $ArrayAgrupacion[1][2] = "03";
        } else {
            $ArrayAgrupacion[1][1] = $Agrupacion;
            $ArrayAgrupacion[1][2] = "05";
        }
        ...
        for ($i = 1; $i <= count($ArrayAgrupacion); $i++) {
            ...
            where a.Cuenta = cuenta.Id and o.Agrupacion = " . $ArrayAgrupacion[$i][1] . " and k.Movimiento = 2 and k.Fecha between '$FechaInicio' and '$FechaFinal' and  a.NumBodega = 1   ) as Conteo  
            from cuenta where Id >0 and Id <> 16   ");
            ...
            <table width="90%" border="1" align="center">
                ...
                for ($j = 1; $j <= count($ArrayCuenta); $j++) {
                    ...
                    where a.Cuenta = " . $ArrayCuenta[$j][1] . " and k.Movimiento = 2 and o.Agrupacion = " . $ArrayAgrupacion[$i][1] . " "
                            . " and k.Fecha between '$FechaInicio' and '$FechaFinal' and a.NumBodega = 1
                    group by a.Id,k.Precio order by a.Descripcion");
					
					
					echo "select m.Descripcion as Medida ,a.Descripcion as Articulo ,ifnull(SUM(Cantidad),0) as Cantidad, k.Precio from kardex k 
                    inner join articulo a ON a.Id = k.Articulo
                    inner join medida m ON m.Id = a.Medida
                    inner join orden o ON o.Id = k.Orden
                    where a.Cuenta = " . $ArrayCuenta[$j][1] . " and k.Movimiento = 2 and o.Agrupacion = " . $ArrayAgrupacion[$i][1] . " "
                            . " and k.Fecha between '$FechaInicio' and '$FechaFinal' and a.NumBodega = 1
                    group by a.Id,k.Precio order by a.Descripcion <br><br>";
